...
describe('readableAddress', () => {
  test('drops what nobody types and keeps what they do', () => {
    ...
    expect(readableAddress('https://www.linkedin.com/in/piglardlord/')).toBe('linkedin.com/in/piglardlord');
    expect(readableAddress('http://piglardlord.github.io/cv-giovanni/')).toBe('piglardlord.github.io/cv-giovanni');
  });

  // A shortened address is a wrong address: the path is the part that identifies the person.
  test('the path survives whole', () => {
    expect(readableAddress('https://sites.google.com/view/giovanni-trovato'))
      .toBe('sites.google.com/view/giovanni-trovato');
  });
