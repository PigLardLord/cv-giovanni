...
// A degree's scope follows its period on the school's line (#48): "School (2014 – 2016) · 60 ECTS". The period is the
// segment after the school that reads as one, wherever it stands, and a segment that does not read as one is neither
// the school nor the period.
describe('a school line that goes on past its period', () => {
  const PISA = 'Università degli Studi di Pisa';

  test.each([
    ['"School (period) · scope"', `${PISA} (2014 – 2016) · 60 ECTS`, PISA, '2014 – 2016'],
    ['"School · period · scope"', `${PISA} · 2014 – 2016 · 60 ECTS`, PISA, '2014 – 2016'],
    ['"School (period)"', `${PISA} (2014 – 2016)`, PISA, '2014 – 2016'],
    ['"School · period"', `${PISA} · 2014 – 2016`, PISA, '2014 – 2016'],
    [
      'a name with a parenthesis, then the period',
      'Technische Universität München (TUM) · 2014 – 2016 · 120 ECTS',
      'Technische Universität München (TUM)',
      '2014 – 2016'
    ]
  ])('%s gives the school and the period', (_, line, school, period) => {
    expect(AtsTextParser.schoolAndPeriod(line)).toEqual({ school, period });
  });

  test.each([`${PISA} · 60 ECTS`, 'Technische Universität München (TUM) · 120 ECTS'])(
    'a segment that is not a period is never taken for one: %s',
    (line) => {
      expect(AtsTextParser.schoolAndPeriod(line)).toEqual({ school: null, period: null });
    }
  );

  const educationOf = (lines) =>
    AtsTextParser.parse(
      [
        'Giovanni Rossi',
        'rossi@example.com',
    ...
    ).education.map((entry) => [entry.degree.value, entry.school.value, entry.period]);

  test('a wrapped degree closed by "School (period) · scope" is one degree, with its school and period', () => {
    expect(
      educationOf([
        "First Level Professional Master's Programme in Mobile Applications",
        'Development',
        `${PISA} (2014 – 2016) · 60 ECTS`,
  ...
  test('a degree whose school line names a scope and no period keeps its school, and no period', () => {
    expect(educationOf(['M.Sc. Informatics', 'Technische Universität München · 120 ECTS'])).toEqual(
      [['M.Sc. Informatics', 'Technische Universität München', null]]
    );
  });
});
