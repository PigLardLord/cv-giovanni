...
describe('monochrome output carries no colour', () => {
  ...
  test.each(registry.supported())(
    '%s in monochrome resolves every colour to a true grey',
    (layout) => {
  ...
  test('the design system takes its text colours from the theme, not from constants', () => {
    ...
    const colours = [
      resolved.defaultStyle.color,
      ...Object.values(resolved.styles).map((s) => s.color)
    ].filter((value) => typeof value === 'string' && value.startsWith('#'));
