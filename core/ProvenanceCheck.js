...
import { ALSO_ENGLISH_WORDS, TECHNOLOGIES } from '../domain/TechnologyLexicon.js';
...
export class ProvenanceCheck {
  ...
  static failures({
    ...
    const vocabulary = [
      ...
      // sentence, or among a translation's capitalised nouns, its capitals say nothing (#292). One that is an English
      // word too is read from its capital in English, and held as a technology in a translation only (the review of
      // #360).
      ...TECHNOLOGIES,
      ...(translated ? ALSO_ENGLISH_WORDS : [])
    ].filter(Boolean);
    ...
    for (const [key, what, kept, dated, wordedFields] of ENTRIES) {
      (tailored[key] || []).forEach((entry, index) => {
        ...
        if (!translated) {
          ...
          // field as the source writes it (#374).
          const shortened = (candidate) =>
            entry.description === undefined || entry.description === null
              ? { ...candidate, description: undefined }
              : candidate;
          if (!(source[key] || []).some((candidate) => same(entry, shortened(candidate)))) {
