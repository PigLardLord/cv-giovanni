  ...
  const findings = {
    ...
    untappable: size.mobile
      ? [
          ...
          ...shownButtons
            .filter((button) => button.height < TAP_TARGET - 1)
            .map((button) => `the ${button.place}'s "${button.label}" renders ${button.height}px`),
  ...
  return {
    ...
    measures: {
      ...
      heights: [...shown, ...shownButtons].length
        ? `${[...shown, ...shownButtons].map((control) => control.height).join(' · ')}px`
        : '—',
