...
period before #200, is listed as not graded on the base, and is not a loss. **The label
`ats-trade-accepted` records a trade the owner accepted:** the losses are still reported, in the
job summary, and the step passes. A change to one side only exits 0 and says why; a base that could
not be built, read or graded exits 2. Locally it compares the working tree with its merge base with
